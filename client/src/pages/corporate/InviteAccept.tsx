...
import { getLoginUrl, getGoogleLoginUrl, getGithubLoginUrl } from "@/const";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Checkbox } from "@/components/ui/checkbox";
import { Skeleton } from "@/components/ui/skeleton";
import { Building2, CheckCircle, AlertTriangle, LogIn, Mail } from "lucide-react";
    ...
    <div className="min-h-screen bg-navy flex items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardContent className="p-8 space-y-6">
          ...
          {!user ? (
            /* Not logged in — show all auth options */
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground text-center">
                Faça login para aceitar o convite.
              </p>
              <Button className="w-full" variant="outline" onClick={() => {
                window.location.href = getGoogleLoginUrl(`/corporate/invite/${token}`);
              }}>
              ...
              <Button className="w-full" variant="outline" onClick={() => {
                window.location.href = getGithubLoginUrl(`/corporate/invite/${token}`);
              }}>
              ...
              <Button className="w-full" onClick={() => {
                ...
                <Mail className="h-4 w-4 mr-2" /> Entrar com Email e Senha
              </Button>
            </div>
          ) : (
